...
- 保持本卷形成的语气、称呼、关系张力、身体状态和互动节奏。`;
        ...
        function getContinuationOnlyArchiveProfilePrompt(profile) {
            const outputTemplate = `请基于全部输入生成仅供模型续写的记忆包。禁止续写剧情，禁止替用户角色做决定，禁止添加原文未发生的内容。\n\n重要原则：请先按输入或分段摘要的顺序串联剧情时间线，再从完整时间线中提取关系、状态、亲密余波和伏笔。不要直接按主题合并所有内容。连续事件必须保持连续。若原文时间标签冲突或存疑，请标注“存疑”，禁止自行理顺或重排。\n\nAI侧角色/角色组说明：该名称可能是单一角色，也可能包含多个由 assistant 同时扮演的角色。请根据角色设定和对话内容判断实际主要角色。若只有一个 AI侧角色，不要凭空拆出多个角色；若有多个 AI侧角色，不要把他们合并成一个人。不要默认一定是多角色，也不要默认一定是单角色；单角色时按单角色总结，多角色时逐人记录行为、心理、身体状态、关系变化和接续状态。不要凭空创造原文没有的角色。\n\n最终续写包必须控制在 2200～2800 中文字以内。不要追求面面俱到，优先保留对下一卷续写最有用的信息。不得把分段摘要机械拼接成长文。\n\n各部分限制：\n- 剧情时间线：最多 10～12 条，每条不超过 60 中文字；必须按原文顺序。\n- 关键关系变化：最多 8 条，写清“从什么变成什么 + 触发事件”。\n- 角色状态继承：每个主要角色最多 3 条，优先写当前心理、身体状态、动机、隐瞒信息。\n- 亲密余波：最多 6 条，只写关系影响、身体/情绪余波，不露骨复述。\n- 未回收伏笔：最多 8 条。\n- 接续点：400～600 中文字，必须详细记录当前地点、时间、人物位置、最后动作/台词/情绪、身体状态、未完成互动和下一轮自然接续点。\n\n如果内容过多，优先级如下：1. 接续点；2. 剧情时间线；3. 关键关系变化；4. 角色状态继承；5. 亲密余波；6. 未回收伏笔。\n\n请严格只输出以下一个分隔块，不要输出 USER_ARCHIVE，不要输出 ARCHIVE_TITLE，不要输出 JSON，不要输出卷标题，不要输出一句话概括，不要输出固定约束段，不要添加分隔符之外的开场白、解释或补充说明。\n\n<<<CONTINUATION_PROMPT>>>\n# 下一卷续写包\n\n## 剧情时间线\n依时序串联全部核心事件，保留原时间标签。最多 10～12 条，每条不超过 60 中文字；冲突处标注“存疑”，禁止理顺或重排。\n\n## 关键关系变化\n最多 8 条，仅列本卷发生的关系状态演变，并注明触发事件。含称呼、边界、占有欲等变化；多角色时按实际角色对分别记录，单角色时不要虚构关系对象。\n\n## 角色状态继承\n逐人记录 AI侧角色组、用户角色、其他主要角色的卷末状态；每个主要角色最多 3 条，优先写心理状态、身体状态、当前动机和隐瞒信息。仅记录原文和设定中实际存在的角色；若只有一个 AI侧角色，不要凭空拆出多个角色；若有多个 AI侧角色，不要把他们合并成一个人。\n\n## 亲密余波\n最多 6 条，只记录亲密行为造成的关系变化、身体痕迹/疲惫、情绪后遗。禁露骨复述，禁推测未发生的反应。\n\n## 未回收伏笔\n最多 8 条，列明未解决的误会、谎言、未完成对话、已预告事件及未响应的线索。\n\n## 接续点\n400～600 中文字，详述卷末：地点、时间、各实际角色的位置与姿势、最后台词/动作、情绪与张力、身体状态、未完成动作。依此指出最直接的接续方向，严禁写出后续剧情。\n<<<END_CONTINUATION_PROMPT>>>`;
...
- 最终续写包必须控制在 2200～2800 中文字以内。不要追求面面俱到，优先保留对下一卷续写最有用的信息。
- 重要原则：请先按分段摘要的顺序串联剧情时间线，再从完整时间线中提取关系、状态、亲密余波和伏笔。不要直接按主题合并所有分段摘要。
- 连续事件必须保持连续；必须覆盖所有分段，不能只总结最后一段。
- 不要机械拼接分段摘要；要去重、合并重复信息，但不得打乱时序。
- 各部分限制：剧情时间线最多 10～12 条，每条不超过 60 中文字；关键关系变化最多 8 条；每个主要角色状态最多 3 条；亲密余波最多 6 条；未回收伏笔最多 8 条；接续点 400～600 中文字。
- 如果空间不足，优先保留：接续点、剧情时间线、关键关系变化、角色状态继承、亲密余波、未回收伏笔。
- 接续点必须相对详细，不能只写一句话；必须记录当前地点、时间、人物位置、最后动作/台词/情绪、身体状态、未完成互动和下一轮自然接续点。

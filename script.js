...
- 不得补写剧情，不得替用户角色决定下一步行为。`;
        ...
        let editingUserMessageIndex = null, activeTab = 'list', editRoleMode = 'free', newRoleMode = 'struct', colorScope = 'global', bgSheetContext = 'chat', optionSheetKind = null, modalState = {}, avatarCropState = null;
        ...
        const UIManager = {
            ...
            applyBgOpacity(value = 0.7) { const opacity = normalizeBgOpacity(value); DOM.device.style.setProperty('--bg-opacity', String(opacity)); if (DOM.bgOpacitySlider) DOM.bgOpacitySlider.value = String(Math.round(opacity * 100)); if (DOM.bgOpacityVal) DOM.bgOpacityVal.textContent = Math.round(opacity * 100) + '%'; },
            ...
            async applyConversationBackground(bgId = 'default', bgOpacity = DataManager.getCurrentConversation()?.bgOpacity) { if (!DOM.customBgLayer) return; this.applyBgOpacity(bgOpacity); if (!bgId || bgId === 'default') { DOM.customBgLayer.style.backgroundImage = 'none'; DOM.customBgLayer.classList.remove('active'); DOM.device.classList.remove('has-custom-bg'); return; } try { const bgs = await DBManager.getAllBgs(); const bg = bgs.find(item => item.id === bgId); if (bg) { DOM.customBgLayer.style.backgroundImage = 'url(' + bg.dataUrl + ')'; DOM.customBgLayer.classList.add('active'); DOM.device.classList.add('has-custom-bg'); } else { DOM.customBgLayer.style.backgroundImage = 'none'; DOM.customBgLayer.classList.remove('active'); DOM.device.classList.remove('has-custom-bg'); } } catch(err) { console.error('加载背景失败', err); } },
